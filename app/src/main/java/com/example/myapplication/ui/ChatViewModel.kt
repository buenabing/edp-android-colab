...
class ChatViewModel(
    ...
    companion object {
        val Factory: ViewModelProvider.Factory = viewModelFactory {
            initializer {
                val context = checkNotNull(this[ViewModelProvider.AndroidViewModelFactory.APPLICATION_KEY])
                val dao = ChatDatabase.getDatabase(context).messageDao()
                ChatViewModel(
                    ChatRepositoryImpl(NetworkModule.chatApi, dao)
                )
            }
        }
    }
